...
public class main {
    ...
    public void customizePizza() {
        ...
        user.setLastPizzaName(pizzaName);
    ...
    public void viewAndReorderFavorites() {
        System.out.print("Enter Your User Id: ");
        String userId = scanner.nextLine();
    }    
    ...
    public void placeOrder() {
        ...
        if (orderChoice == 1) {
            ...
            if (paymentChoice == 1) {
                ...
                System.out.println("Card Number: " + cardNumber); // Use the cardNumber variable
        ...
        if (orderChoice == 2) {
          ...
          // Fetch the user object from the collection
           if (!userProfiles.containsKey(userId)) {
           System.out.println("User not found! Please create a profile first.");
         return; 
         }
           ...
           User user = userProfiles.get(userId);
            ...
            int pointsEarned = (int) (totalFee / 10); // Convert dollars spent to points
            user.addLoyaltyPoints(pointsEarned); 

            System.out.println("You earned " + pointsEarned + " loyalty points!");
    ...
    if (redeemChoice.equalsIgnoreCase("yes")) {
        ...
        // Convert points to a dollar discount (e.g., 10 points = $1)
        double discount = pointsToRedeem / 10.0;

        if (user.getLoyaltyPoints() >= pointsToRedeem) {
            user.redeemLoyaltyPoints(pointsToRedeem);
            totalFee -= discount; // Apply the discount to the total fee
    ...
    // ... continue with payment and order finalization
    System.out.println("Total Fee after loyalty discount: $" + totalFee);
    ...
    public void viewDetails() {
        ...
        String topping = user.getLastTopping();
        String cheese = user.getLastCheese();
        String pizzaName = user.getLastPizzaName();
